...
pub fn parse_ima_binary_log(log_data: &[u8]) -> Result<Vec<ImaTemplateEntry>, PluginError> {
    ...
    log::info!("Parsing IMA binary runtime measurement log ({} bytes)", log_data.len());
    
    while cursor.position() < log_data.len() as u64 {
        let entry = parse_ima_entry(&mut cursor)?;
        log::debug!("Parsed IMA entry: PCR={}, template={}, data_len={}", 
                   entry.pcr_index, entry.template_name, entry.template_data_len);
        entries.push(entry);
    }
    
    log::info!("Successfully parsed {} IMA template entries", entries.len());
    Ok(entries)
}
...
async fn verify_runtime_measurement(
    ...
    log::info!("Starting runtime measurement verification using ImaLog verify function");
    ...
    log::info!("Runtime measurement verification result: log_status={}, ref_value_match_status={}", 
               result.log_status, result.ref_value_match_status);
    Ok(result)
}
...
pub async fn verify_all_logs(
    ...
    log::info!("Verified {} logs, {} with successful replay", results.len(), 
               results.iter().filter(|r| r.log_status == "replay_success").count());
